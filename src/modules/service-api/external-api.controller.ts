...
export class ExternalApiController {
  ...
  async createPost(@Body() body: ExternalCreatePostDto, @Req() req: any) {
    const actor = await this.actorResolver.resolveWritableActor(body, req.externalApiKey);
    ...
    return { id: post?.id, type: 'post', status: post?.status, actor_user_id: actor.id, post };
  }
  ...
  async updatePost(
    ...
    await this.auditOperation(req, 'posts.update', 'posts:write', actor.id, 'post', id, {
      status: post?.status,
      title: post?.title,
    });

    return { id, type: 'post', status: post?.status, actor_user_id: actor.id, post };
  }

  @Delete('posts/:id')
  @ExternalScope('posts:delete')
  async deletePost(@Param('id', ParseIntPipe) id: number, @Query() query: any, @Req() req: any) {
    const actor = await this.actorResolver.resolveWritableActor(query, req.externalApiKey);
    await this.postsService.softDelete(id, actor.id, 'admin');
    await this.auditOperation(req, 'posts.delete', 'posts:delete', actor.id, 'post', id);
  ...
  async createReply(
    ...
    return { id: reply.id, type: 'reply', status: reply.status, actor_user_id: actor.id, reply };
  }
  ...
  async updateReply(
    ...
    const actor = await this.actorResolver.resolveWritableActor(body, req.externalApiKey);
    const reply = await this.repliesService.update(id, body.content, actor.id, 'admin');
    await this.auditOperation(req, 'replies.update', 'replies:write', actor.id, 'reply', id, { post_id: reply.post_id });
    return { id, type: 'reply', status: reply.status, actor_user_id: actor.id, reply };
  }

  @Delete('replies/:id')
  @ExternalScope('replies:delete')
  async deleteReply(@Param('id', ParseIntPipe) id: number, @Query() query: any, @Req() req: any) {
    const actor = await this.actorResolver.resolveWritableActor(query, req.externalApiKey);
    await this.repliesService.softDelete(id, actor.id, 'admin');
    await this.auditOperation(req, 'replies.delete', 'replies:delete', actor.id, 'reply', id);
  ...
  async moderatePost(
    ...
    const actor = await this.actorResolver.resolveActor(body, req.externalApiKey);
    ...
    switch (body.action) {
      ...
      case 'lock':
      case 'unlock':
        result = await this.postsService.setLocked(id, body.action === 'lock', { id: actor.id, role: 'admin' });
        break;
      case 'move':
        result = await this.postsService.move(id, Number(body.category_id));
        break;
      case 'best_reply':
        result = await this.postsService.setBestReply(id, Number(body.reply_id), { id: actor.id, role: 'admin' });
        break;
      case 'clear_best_reply':
        result = await this.postsService.setBestReply(id, null, { id: actor.id, role: 'admin' });
        break;
    }

    await this.auditOperation(req, `posts.moderation.${body.action}`, 'posts:moderate', actor.id, 'post', id, {
      action: body.action,
      reason: body.reason,
      category_id: body.category_id,
    ...
    return { id, type: 'post', action: body.action, actor_user_id: actor.id, result };
  }
  ...
  async moderateReply(
    ...
    await this.auditOperation(req, `replies.moderation.${body.action}`, 'posts:moderate', actor.id, 'reply', id, {
      action: body.action,
      reason: body.reason,
    });
  ...
  async listResources(@Query() query: any, @Req() req: any) {
    const scope = hasExternalScope(req.externalApiKey.scopes, 'resources:moderate') ? 'admin' : 'public';
    return this.resourcesService.getList(query, { scope });
  }

  @Post('resources')
  @ExternalScope('resources:write')
  async createResource(@Body() body: ExternalCreateResourceDto, @Req() req: any) {
    const actor = await this.actorResolver.resolveWritableActor(body, req.externalApiKey);
    ...
    await this.auditOperation(req, 'resources.create', 'resources:write', actor.id, 'resource', resource.id, {
      status: resource.status,
      title: resource.title,
    });

    return { id: resource.id, type: 'resource', status: resource.status, actor_user_id: actor.id, resource };
  }
  ...
  async getResource(@Param('id', ParseIntPipe) id: number, @Req() req: any) {
    const viewer = hasExternalScope(req.externalApiKey.scopes, 'resources:moderate') ? this.staffViewer(req) : undefined;
  ...
  async updateResource(
    ...
    const actor = await this.actorResolver.resolveWritableActor(body, req.externalApiKey);
    const resource = await this.resourcesService.update(id, actor.id, {
      title: body.title,
      description: body.description,
      resource_type: body.resource_type,
      external_url: body.external_url,
    ...
    await this.auditOperation(req, 'resources.update', 'resources:write', actor.id, 'resource', id, {
      status: resource.status,
      title: resource.title,
    });

    return { id, type: 'resource', status: resource.status, actor_user_id: actor.id, resource };
  }

  @Delete('resources/:id')
  @ExternalScope('resources:delete')
  async deleteResource(@Param('id', ParseIntPipe) id: number, @Query() query: any, @Req() req: any) {
    const actor = await this.actorResolver.resolveWritableActor(query, req.externalApiKey);
    await this.resourcesService.delete(id, actor.id, 'admin');
    await this.auditOperation(req, 'resources.delete', 'resources:delete', actor.id, 'resource', id);
  ...
  async moderateResource(
    ...
    const resource = await this.resourcesService.updateStatus(id, status, { actorUsername: actor.username });
    await this.auditOperation(req, `resources.moderation.${body.action}`, 'resources:moderate', actor.id, 'resource', id, {
      action: body.action,
      status,
    });
    return { id, type: 'resource', action: body.action, status: resource.status, actor_user_id: actor.id, resource };
  }
  ...
  private async auditOperation(
    ...
    await this.auditService.recordOperation({
      ...
      details,
    }).catch((error) => console.warn('external api audit failed:', error.message));
  }
