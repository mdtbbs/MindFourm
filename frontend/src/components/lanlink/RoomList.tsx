...
function forumUserName(room: PublicRoom): string {
  return present(room.owner.forum_display_name ?? room.owner.display_name ?? room.owner.forum_username ?? room.owner.username);
}

function gamePlayerId(room: PublicRoom): string {
  return present(room.owner.game_player_id ?? room.owner.player_id ?? room.game_player_id ?? room.player_id);
}
...
export default function RoomList() {
  ...
  const fetchRooms = useCallback(async (manual = false) => {
    ...
    try {
      ...
      setRooms(Array.isArray(data.rooms) ? data.rooms.filter((room) => room.public === true) : []);
      setError(null);
    } catch (err) {
  ...
  useEffect(() => {
    void fetchRooms();
    const timer = window.setInterval(() => void fetchRooms(), POLL_INTERVAL);
  ...
  if (loading) {
    return <div className="card py-12 text-center text-muted-foreground">正在加载在线房间…</div>;
  }
    ...
    <section aria-labelledby="online-rooms-title" className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 id="online-rooms-title" className="text-xl font-bold">公开房间</h2>
      ...
      {rooms.length === 0 ? (
        <div className="card py-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">房间上线后会在此处自动显示。</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2">
          {rooms.map((room) => {
            ...
            return (
              <article key={room.code} className="card flex min-w-0 flex-col gap-4 p-5">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    <h3 className="truncate text-lg font-bold">{room.name || room.display_name || '未命名房间'}</h3>
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                      {nodeName && <span>节点：{nodeName}</span>}
                      {hasPlayerCount && (
                        <span>在线：{room.players}{typeof room.max_players === 'number' ? ` / ${room.max_players}` : ''}</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="rounded-lg bg-muted/60 p-3">
                  <p className="mb-1 text-xs font-medium text-muted-foreground">MOTD</p>
                  <p className="max-h-24 overflow-auto whitespace-pre-wrap break-words text-sm">{room.motd || room.display_name || '房主未填写 MOTD'}</p>
                </div>

                <dl className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-2 text-sm">
                  <dt className="text-muted-foreground">论坛用户</dt>
                  <dd className="min-w-0 break-all font-medium">{forumUserName(room)}</dd>
                  <dt className="text-muted-foreground">论坛 ID</dt>
                  <dd className="min-w-0 break-all font-mono">{forumUserId(room)}</dd>
                  <dt className="text-muted-foreground">游戏名称</dt>
                  <dd className="min-w-0 break-all font-medium">{gamePlayerName(room)}</dd>
                  <dt className="text-muted-foreground">游戏玩家 ID</dt>
                  <dd className="min-w-0 break-all font-mono text-xs">{gamePlayerId(room)}</dd>
                </dl>
                ...
                <div className="mt-auto flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
                  ...
                  {lastUpdated && <p className="w-full text-right text-[11px] text-muted-foreground">最后更新：{lastUpdated}</p>}
                </div>
              </article>
            );
          })}
