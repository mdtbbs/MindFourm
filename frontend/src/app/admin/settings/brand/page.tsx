...
export default function BrandSettingsPage() {
  ...
  const handleSidebarLogoUpload = async (file: File) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : '上传侧边栏 Logo 失败');
    } finally {
      setUploadingSidebarLogo(false);
      if (sidebarLogoInputRef.current) {
        sidebarLogoInputRef.current.value = '';
      }
    }
  };
  ...
  return (
    <div className="bg-white border border-surface-200">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="border-t border-surface-200 pt-6">
          <h3 className="text-sm font-semibold text-surface-800">侧边栏 Logo</h3>
          <p className="mt-1 text-xs text-surface-400">侧边栏顶部品牌区域显示的专用logo，支持长条形图片</p>
        </div>

        <div className="grid gap-4 md:grid-cols-[160px_1fr]">
          <div>
            <div className="flex h-16 w-36 items-center justify-center overflow-hidden rounded border border-surface-200 bg-surface-50">
              {values.sidebar_logo_url ? (
                <img src={values.sidebar_logo_url} alt="侧边栏 Logo 预览" className="max-h-12 max-w-full object-contain" />
              ) : (
                <span className="px-3 text-center text-xs text-surface-400">未设置</span>
              )}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-surface-600 mb-2">侧边栏 Logo 地址</label>
              <input
                className="w-full px-3 py-2 border border-surface-200 rounded text-sm focus:outline-none focus:border-surface-400"
                value={values.sidebar_logo_url ?? ''}
                onChange={(e) => update('sidebar_logo_url', e.target.value)}
                placeholder="/uploads/public-images/sidebar-logo.png"
              />
              <p className="text-xs text-surface-400 mt-1">留空则使用站点 Logo；推荐使用横向长条形图片，高度 32-48px</p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file) void handleSidebarLogoUpload(file);
                }}
                disabled={uploadingSidebarLogo}
              />
              <Button type="button" variant="ghost" onClick={() => sidebarLogoInputRef.current?.click()} disabled={uploadingSidebarLogo}>
                {uploadingSidebarLogo ? '上传中...' : '上传图片'}
              </Button>
              {values.sidebar_logo_url ? (
                <Button
                  type="button"
                  variant="ghost"
                  onClick={() => {
                    update('sidebar_logo_url', '');
                    updateGlobalSetting('sidebar_logo_url', '');
                  }}
                  disabled={uploadingSidebarLogo}
                >
                  清空
